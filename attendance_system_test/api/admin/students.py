...
from core.db import get_db, get_cursor, get_students
...
def list_students():
    ...
    if section_id or search or enrollment_filter or student_id:
        ...
        query += " ORDER BY s.usn LIMIT 200"
    ...
    # Fetch all active departments and all sections across all departments
    cursor.execute("""
        SELECT sec.id, sec.section_label, sec.sem_number, sec.department_id, d.code AS dept_code, d.name AS dept_name
        FROM sections sec
        JOIN departments d ON d.id = sec.department_id
...
def assign_section(student_id):
    """Assign or transfer a student's section with semester eligibility check (<= current sem)."""
    section_id = request.form.get("section_id", "")

    if not section_id:
        flash("Section is required.", "danger")
        return redirect(request.referrer or url_for("students_mgmt.list_students"))
    ...
    try:
        ...
        cursor.execute("SELECT id, current_sem, usn FROM students WHERE id = %s", (student_id,))
        student_info = cursor.fetchone()
        if not student_info:
            flash("Student not found.", "danger")
            return redirect(request.referrer or url_for("students_mgmt.list_students"))

        curr_sem = student_info["current_sem"] or 8

        cursor.execute("""
            SELECT sec.id, sec.sem_number, sec.section_label, d.code AS dept_code
            FROM sections sec
            JOIN departments d ON d.id = sec.department_id
            WHERE sec.id = %s
        """, (int(section_id),))
        ...
        # ENFORCE SEMESTER ELIGIBILITY RULE: target sem <= current sem
        if sec["sem_number"] > curr_sem:
            flash(f"❌ Transfer Blocked: Student is currently in Sem {curr_sem} and cannot be transferred to a higher semester section ({sec['dept_code']} Sem {sec['sem_number']} Section {sec['section_label']}).", "danger")
            return redirect(request.referrer or url_for("students_mgmt.list_students"))

        cursor.execute(
            "UPDATE students SET section_id = %s, current_sem = %s WHERE id = %s",
            (sec["id"], sec["sem_number"], student_id)
        )
        conn.commit()

        log_audit(
            current_user.id, "transfer_student_section",
            "students", student_id, None,
            f"Transferred to {sec['dept_code']} Sem {sec['sem_number']} Section {sec['section_label']}",
            ip_address=get_client_ip()
        )
        flash(f"✅ Student {student_info['usn']} transferred cleanly to {sec['dept_code']} Sem {sec['sem_number']} Section {sec['section_label']} with 0 data loss.", "success")
    except Exception as e:
        if conn:
            conn.rollback()
        flash(f"Error transferring section: {e}", "danger")
...
def edit_student(student_id):
    """Edit a student's basic details and optional section transfer."""
    ...
    if not full_name or not email or not usn:
        ...
        return redirect(request.referrer or url_for("students_mgmt.list_students", student_id=student_id))
    ...
    try:
        ...
        cursor.execute("SELECT user_id, current_sem FROM students WHERE id = %s", (student_id,))
        st = cursor.fetchone()
        if not st:
            flash("Student not found.", "danger")
            return redirect(request.referrer or url_for("students_mgmt.list_students"))
        ...
        if section_id:
            ...
            if sec_info:
                sec_sem = sec_info["sem_number"]
                curr_sem = st["current_sem"] or 8
                if sec_sem > curr_sem:
                    flash(f"❌ Transfer Blocked: Cannot transfer student to higher semester {sec_sem} from Sem {curr_sem}.", "danger")
                    return redirect(request.referrer or url_for("students_mgmt.list_students"))

                cursor.execute(
                    "UPDATE students SET usn=%s, current_sem=%s, section_id=%s WHERE id=%s",
                    (usn, sec_sem, int(section_id), student_id)
                )
        else:
            cursor.execute(
