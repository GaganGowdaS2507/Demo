        ...
        def decorated_function(*args, **kwargs):
            ...
            if not current_user.is_authenticated:
                ...
                if (
                    ...
                    or request.path.endswith("/request-proxy")
                    or request.path.endswith("/claim-proxy")
                ):
...
def log_audit(user_id, action,
    ...
    try:
        ...
        # The connection is request-scoped (shared via g.db) and torn down
        # by close_db() at the end of the request — never close it here.
        # Doing so previously killed the connection out from under every
        # caller that continued using it after calling log_audit().
        try:
            if cursor:
                cursor.close()
        except Exception:
