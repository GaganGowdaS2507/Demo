...
EXPECTED_EMBEDDING_DIM = 512  # MobileFaceNet (w600k_mbf.onnx) output size

def deserialize_embedding(blob):
    """Deserialize embedding from DB blob. Returns None (not a malformed
    array) if the decoded vector isn't the expected dimension, so callers
    never receive an unusable embedding."""
    if blob is None:
        return None
    arr = None
    try:
    ...
    if arr is None or arr.shape[0] != EXPECTED_EMBEDDING_DIM:
        logger.warning(
            "deserialize_embedding: got dim %s, expected %s — skipping",
            arr.shape[0] if arr is not None else None, EXPECTED_EMBEDDING_DIM,
        )
        return None
