...
def list_faculty():
    ...
    # Fetch department-wise faculty counts for department cards view
    cursor.execute("""
        SELECT 
            d.id, 
            d.code, 
            ...
            SUM(CASE WHEN u.profile_completed = 1 THEN 1 ELSE 0 END) AS active_count,
            SUM(CASE WHEN u.profile_completed = 0 THEN 1 ELSE 0 END) AS pending_count
        FROM departments d
        LEFT JOIN faculty f ON f.department_id = d.id
        LEFT JOIN users u ON u.id = f.user_id AND u.role = 'faculty'
        GROUP BY d.id, d.code, d.name
        ORDER BY d.code
    """)
    dept_cards = cursor.fetchall()
    ...
    cursor.execute("""
        SELECT 
            ...
            SUM(CASE WHEN u.profile_completed = 0 THEN 1 ELSE 0 END) AS pending_count
        FROM faculty f
        JOIN users u ON u.id = f.user_id AND u.role = 'faculty'
