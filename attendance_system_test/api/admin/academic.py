...
def refresh_section_academics(section_id):
    ...
    # ---------------------------------------------------
    # STEP 3 : Load Elective Subjects
    # ---------------------------------------------------

    elective_subjects = []

    cursor.execute("SELECT has_clusters FROM departments WHERE id = %s", (department_id,))
    dept_info = cursor.fetchone()
    dept_has_clusters = bool(dept_info.get("has_clusters", 0)) if dept_info else False

    if dept_has_clusters and cluster_id:
        cursor.execute("""
    ...
    elif not dept_has_clusters:
        cursor.execute("""
            SELECT id
            FROM subjects
            WHERE department_id=%s
              AND sem_number=%s
              AND subject_type LIKE 'Elective%%'
...
def add_department():
    ...
    has_clusters = 1 if request.form.get("has_clusters") == "1" else 0
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO departments (code, name, has_clusters, created_at) VALUES (%s, %s, %s, NOW())",
            (code, name, has_clusters)
        )
        conn.commit()
        log_audit(
            current_user.id, "add_department",
            "departments", cursor.lastrowid, None, f"{code}: {name} (Clusters: {has_clusters})",
...
def edit_department(dept_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE departments SET code = %s, name = %s, has_clusters = %s WHERE id = %s",
            (code, name, has_clusters, dept_id)
        )
        conn.commit()
        log_audit(
            current_user.id, "edit_department",
            "departments", dept_id, None, f"{code}: {name} (Clusters: {has_clusters})",
            ip_address=get_client_ip()
        )
...
def department_workspace(dept_id):
    ...
    cursor.execute(
        "SELECT id, code, name, has_clusters, created_at FROM departments WHERE id = %s",
        (dept_id,)
    )
...
@academic_bp.route("/subjects/<int:subject_id>/edit", methods=["POST"])
@login_required
@admin_required
def edit_subject(subject_id):
    ...
    subject_type = request.form.get("subject_type", "Theory").strip()
    offering_mode = request.form.get("offering_mode", "regular").strip()
    credits_val = request.form.get("credits", 0)

    if not code or not name:
        flash("Subject code and name are required.", "warning")
        return redirect(url_for("academic.subjects", selected_subject=subject_id))
    ...
    try:
        ...
        sem = int(sem_number) if sem_number and sem_number.isdigit() else None
        dept_id = int(department_id) if department_id and department_id.isdigit() else None
        cred = int(credits_val) if credits_val and str(credits_val).isdigit() else 0

        cursor.execute(
            """
            UPDATE subjects
            SET code = %s, name = %s, sem_number = %s, department_id = %s,
                subject_type = %s, offering_mode = %s, credits = %s
            WHERE id = %s
            """,
            (code, name, sem, dept_id, subject_type, offering_mode, cred, subject_id)
        )
        conn.commit()
        log_audit(
            current_user.id, "edit_subject",
            "subjects", subject_id, None, f"Updated subject {code} - {name}",
            ip_address=get_client_ip()
        )
...
@academic_bp.route("/section-subjects/<int:section_id>")
@login_required
@admin_required
def section_subjects(section_id):
    """View and manage subject assignments for a section."""
    cursor = get_cursor()
    cursor.execute("""
        SELECT sec.id, sec.section_label, d.code AS dept_code, d.name AS dept_name,
    ...
    cursor.execute("""
        SELECT ss.id, ss.section_id, ss.subject_id, ss.faculty_id, ss.is_elective,
               sub.code AS subject_code, sub.name AS subject_name, sub.subject_type,
               u.full_name AS faculty_name
        FROM section_subjects ss
        JOIN subjects sub ON sub.id = ss.subject_id
        LEFT JOIN faculty f ON f.id = ss.faculty_id
    ...
    cursor.execute("""
        SELECT f.id, u.full_name, d.code AS dept_code
        FROM faculty f
        JOIN users u ON u.id = f.user_id
