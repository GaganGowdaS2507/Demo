    ...
    for _, group in groups.items():
        for i in range(len(group)):
            for j in range(i + 1, len(group)):
                ...
                ca, cb = normalize_text(a["code"]), normalize_text(b["code"])

                is_exact_code = (ca == cb and len(ca) > 0)
                is_exact_name = (na == nb and len(na) > 0)

                # Acronym hit: short code (<= 6 chars) matches full subject name acronym
                acr_hit = (
                    (len(cb) <= 6 and build_acronym(a["name"]) == cb and len(a["name"]) > 6) or
                    (len(ca) <= 6 and build_acronym(b["name"]) == ca and len(b["name"]) > 6)
                )

                if is_exact_code or is_exact_name or acr_hit:
                    score = difflib.SequenceMatcher(None, na, nb).ratio()
                    suggestions.append({
                        "subject_a": a, "subject_b": b,
                        "similarity": 1.0 if (is_exact_code or is_exact_name) else round(score, 2),
                        "acronym_match": acr_hit,
                    })
