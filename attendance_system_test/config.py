...
class Config:
    ...
    # ----- Mobile (MobileFaceNet) dual-embedding model -----
    # Must be the SAME w600k_mbf.onnx bundled in the React Native app,
    # so backend-enrolled embeddings live in the same space as the mobile model.
    MBF_MODEL_PATH = os.getenv(
        "MBF_MODEL_PATH",
        os.path.join(BASE_DIR, "recognition", "models", "w600k_mbf.onnx")
    )
    ...
    # ----- Continue Portal API Integration Settings -----
    CONTINUE_API_BASE_URL   = os.getenv("CONTINUE_API_BASE_URL", "")
    CONTINUE_API_KEY        = os.getenv("CONTINUE_API_KEY", "")
    CONTINUE_AUTH_TYPE      = os.getenv("CONTINUE_AUTH_TYPE", "bearer")
    CONTINUE_SYNC_ENABLED   = os.getenv("CONTINUE_SYNC_ENABLED", "false").lower() == "true"
    CONTINUE_SYNC_MODE      = os.getenv("CONTINUE_SYNC_MODE", "realtime")
    CONTINUE_MAX_RETRIES    = int(os.getenv("CONTINUE_MAX_RETRIES", "5"))
    CONTINUE_API_TIMEOUT    = int(os.getenv("CONTINUE_API_TIMEOUT", "10"))

    # Parameter Equalizer Field Names
    CONTINUE_STUDENT_ID_FIELD   = os.getenv("CONTINUE_STUDENT_ID_FIELD", "usn")
    CONTINUE_STUDENT_FIELD_NAME = os.getenv("CONTINUE_STUDENT_FIELD_NAME", "roll_no")
    CONTINUE_SUBJECT_FIELD      = os.getenv("CONTINUE_SUBJECT_FIELD", "subject_code")