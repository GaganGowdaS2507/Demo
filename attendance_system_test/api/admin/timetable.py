...
def view_timetable():
    ...
    if section_id and not department_id:
        cursor.execute("SELECT department_id, sem_number FROM sections WHERE id = %s", (section_id,))
        sec_meta = cursor.fetchone()
        if sec_meta:
            department_id = str(sec_meta["department_id"])
            if not sem_number:
                sem_number = str(sec_meta["sem_number"])
