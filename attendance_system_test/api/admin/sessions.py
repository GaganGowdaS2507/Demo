...
@admin_required
def create_session():
    creator_faculty_id = _current_faculty_id()  # non-None only if a faculty user is creating their own session
    all_sections = _get_sections_for_role(creator_faculty_id)
    ...
    form_faculty_id    = request.form.get("faculty_id", type=int)

    # Resolve which faculty this session is assigned to BEFORE any
    # validation reads it — a faculty user always gets their own id,
    # an admin gets whichever faculty was actually selected in the form.
    if current_user.role == "faculty":
        faculty_id = creator_faculty_id
    else:
        faculty_id = form_faculty_id
    ...
    if current_user.role != "faculty" and not faculty_id:
        errors.append("Select a faculty member to assign this session to.")
    ...
    # Only restrict section choice when the CREATOR is a faculty user
    # picking sections for their own class — never for admin, and never
    # based on the target faculty's own assignments (that's a separate,
    # legitimate business rule you may want to add later, but conflating
    # it with this check was incorrect).
    if creator_faculty_id:
    ...
    try:
        ...
        try:
            ...
            logger.error("create_session: rollback also failed: %s", rollback_exc)
        flash(f"Database error while creating session: {exc}", "danger")
        return redirect(url_for("sessions.create_session"))
...
def assign_camera(session_id):
    ...
    raw_source = request.form.get("rtsp_url", "").strip() or None
    conn = None
    try:
