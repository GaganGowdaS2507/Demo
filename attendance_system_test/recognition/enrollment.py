...
class EnrollmentManager:
    ...
    def enroll_from_image(self, pil_image, student_id, usn, db_pool,
                          enrollment_method="single_photo",rebuild_index=True):
        """
        ...
        # Compute embedding
        emb, mbf_emb,face_crop_pil = self.engine.compute_dual_embeddings_from_pil(pil_image)
        if emb is None:
        ...
        try:
            ...
            if mbf_emb is not None:
                mbf_blob = serialize_embedding(mbf_emb)

                cursor.execute(
                    """
                    INSERT INTO faces_mobile
                        (student_id, usn, embedding, model_version, created_at)
                    VALUES
                        (%s, %s, %s, %s, NOW())
                ...
                logger.info(f"Stored MBF embedding for {usn}")

            else:
                logger.warning(f"MBF embedding generation failed for {usn}")
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    ...
                    logger.info(f"📥 Download time for {sid}: {time.time()-t:.2f}s")
                    ...
                    logger.info(f"🧠 Enrollment time for {sid}: {time.time()-t:.2f}s")
            ...
            # Rebuild recognition index only once after all enrollments
            self.engine.reload_embedding_index(db_pool)
            self.engine.refresh_student_cache(db_pool)
